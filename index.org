...
POV is simular to LOD because it establishes the distance of
an observer to what is being defined, but where LOD is a
measure of how much detail can be included or observed, POV
includes the orientation of an observer relative to what is
being defined.

A POV can be thought of as a filter that determines what can
be obseved.  For example, if an observer is looking at 
