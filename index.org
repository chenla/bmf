...
given horcrux.
   ...
   if so, a horcrux is functionally a host -- which is not
   such a stretch here -- but you should not be able to
   traceroute a horcrux -- pings are broadcast, and listened
   for by horcruxes -- and when horcruxes are created there
   shouldn't be any way to know where they are hosted.
